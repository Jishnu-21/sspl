...
const PerkCard: React.FC<PerkCardProps> = ({ icon, title, bgColor, delay }) => {
  return (
    <motion.div 
      className={`${bgColor} rounded-lg p-6 flex flex-col items-center justify-center h-[120px] md:h-[150px]`}
      initial={{ opacity: 0, y: 20 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true, margin: "-50px" }}
      transition={{ duration: 0.5, delay }}
    >
      <div className="mb-3 text-[#1B3D69]">
        {icon}
      </div>
      <p className="text-center text-sm md:text-base font-medium text-gray-800">{title}</p>
    </motion.div>
  );
};
...
const CareerPerks = () => {
  const perks = [
    ...
    },
    {
      ...
      title: "Extravagant Annual Events",
      bgColor: "bg-[#F5F5FF]"
    }
  ];
